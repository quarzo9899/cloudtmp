...
public class CategoriaController {
	  ...
	  public String getCategorie(@RequestHeader("Token") String Token) {
		  ...
		  List<String> categorieNome = new ArrayList<String>();
		  for(Categoria categoria:categorie) {
			  categorieNome.add(categoria.getNome());
		  }
	  ...
	  public Categoria createCategorie(@Valid @RequestBody Categoria categoriaRequest, @RequestHeader("Token") String Token) {
		  String tokenDecrypted = Decrypt.Decrypt(Token);
		  Gson gson = new Gson();
		  MessageBody msg = gson.fromJson(tokenDecrypted, MessageBody.class);
		  if(!isTokenValid(msg.expire)) {
	  ...
	  public Categoria updateCategoria(@PathVariable String categoriaName, @RequestHeader("Token") String Token, @Valid @RequestBody Categoria categoriaRequest) {
		  String tokenDecrypted = Decrypt.Decrypt(Token);
		  Gson gson = new Gson();
		  MessageBody msg = gson.fromJson(tokenDecrypted, MessageBody.class);
		  if(!isTokenValid(msg.expire))
	  ...
	  public ResponseEntity<?>  deleteCategoria(@PathVariable String categoriaName, @RequestHeader("Token") String Token, @Valid @RequestBody Categoria categoriaRequest) {
		  String tokenDecrypted = Decrypt.Decrypt(Token);
		  Gson gson = new Gson();
		  MessageBody msg = gson.fromJson(tokenDecrypted, MessageBody.class);
		  if(!isTokenValid(msg.expire))
