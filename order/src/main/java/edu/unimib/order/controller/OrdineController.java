...
public class OrdineController {
	@Autowired private OrdineRepository ordineRepository;
	  ...
	  @PostMapping("/ordini")
	  public List<Ordine> postOrdine(@Valid @RequestBody Ordine ordineRequest, @RequestHeader("Token") String Token) {
		  String tokenDecrypted = Decrypt.Decrypt(Token);
		  Gson gson = new Gson();
		  MessageBody msg = gson.fromJson(tokenDecrypted, MessageBody.class);
		  if(!isTokenValid(msg.expire))
	  ...
	  private boolean isProdottiAvailables(List<Integer> prodottiId) {
		  ...
		  for(Integer prodottoId: uniqueProdottiId) {
			  ...
			  if(!isProdottoAvailable(prodottoId, occurrences))
				  return false;
		  }
		  return true;
	  }
	  ...
	  public Ordine updateOrdine(@PathVariable long ordineId, @Valid @RequestBody Ordine ordineRequest, @RequestHeader("Token") String Token) {
		  String tokenDecrypted = Decrypt.Decrypt(Token);
		  Gson gson = new Gson();
		  MessageBody msg = gson.fromJson(tokenDecrypted, MessageBody.class);
		  if(!isTokenValid(msg.expire))
		...
		return ordineRepository.findById(ordineId).map(
				ordine -> {
	  ...
	  public ResponseEntity<?> deleteOrdine(@PathVariable long ordineId, @RequestHeader("Token") String Token) {
		  String tokenDecrypted = Decrypt.Decrypt(Token);
		  Gson gson = new Gson();
		  MessageBody msg = gson.fromJson(tokenDecrypted, MessageBody.class);
		  if(!isTokenValid(msg.expire))
